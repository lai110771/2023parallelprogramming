...
/*define the name of thread and mutex
*/
std::thread threads[THREAD_NUM];
std::mutex mutex;

void calculate_image_data(int &buffer){
    mutex.lock();
    int local_task_id = task_id++;
    mutex.unlock();

    for(int x = local_task_id*IMAGE_WIDTH/THREAD_NUM; x < (local_task_id+1)*IMAGE_WIDTH/THREAD_NUM; x++) {
        for(int y = IMAGE_HEIGHT - 1; y >= 0; y--) {
            ...
            mutex.lock();
            auto output_color = compute_color(checksum, pixel_color, NUM_SAMPLES);
            mutex.unlock();
...
int main(int argc, char **argv) {
    ...
    for (int thread_id = 0; thread_id < THREAD_NUM; thread_id++){
        threads[thread_id] = std::thread(calculate_image_data, std::ref(buffer[thread_id]));
    }

    for (int thread_id = 0; thread_id < THREAD_NUM; thread_id++){
        threads[thread_id].join();
        *image_data += buffer[thread_id];
